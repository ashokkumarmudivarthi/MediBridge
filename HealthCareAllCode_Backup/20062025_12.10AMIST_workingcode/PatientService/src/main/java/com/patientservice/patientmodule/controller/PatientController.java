 ...
 // NEW: Update patient endpoint using PatientUpdateRequest DTO
   ...
   /* @GetMapping("/user/{userId}")
    public ResponseEntity<ApiResponse<Patient>> getByUserId(@PathVariable Long userId) {
        Optional<Patient> patientOpt = patientService.getByUserIdOptional(userId);
        return patientOpt.map(patient ->
            ResponseEntity.ok(new ApiResponse<>(true, "Patient fetched", patient))
        ).orElseGet(() ->
            ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ApiResponse<>(false, "Patient not found", null))
        );
    }
    */
